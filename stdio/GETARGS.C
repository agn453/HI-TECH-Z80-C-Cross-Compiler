...
#define	MAXLEN	255		/* max length of an argument */
...
extern void	bmove(void *, void *, unsigned);
	...
	while(look()) {
		...
		*ap = 0;
		if(iswild(buf)) {
			...
			c = getuid();
			setuid(fc.uid);
			if((j = (unsigned char)bdos(CPMFFST, & fc)) != 255)
			do {
				ap = buf;
				if(quote) {
					sprintf(ap, "%u:", fc.uid);
			...
			} while((j = (unsigned char)bdos(CPMFNXT)) != 255);
			else {
				setuid(c);
